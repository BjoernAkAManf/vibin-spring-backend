...
public class QueueImplIntegrationTest {
    ...
    public void multi() {
        ...
        for (int i = 0; i < n; i += 1) {
            int prefix = i;
            final Thread thread = this.createThread(() -> this.join("a" + prefix, "b" + prefix, errors), errors);
            threads.add(thread);
            thread.start();
        }
        ...
        for (int i = 0; i < n; i += 1) {
            final Optional<QueueMatch> opt = this.queue.pollMatch();
    ...
    private void runSimulation(final String u1, final String u2) {
        ...
        final QueueMatch match = opt.get();
    ...
    private void join(final String u1, final String u2, final List<Throwable> errors) {
        assertThatCode(() -> {
            final Thread t1 = this.createThread(() -> this.queue.join(u1), errors);
            final Thread t2 = this.createThread(() -> this.queue.join(u2), errors);
